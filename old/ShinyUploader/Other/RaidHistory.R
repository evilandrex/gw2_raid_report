...
allData <- read.table(file = 'alldata.txt', header = FALSE, col.names = c(colNames, buffNames), sep = '\t')
allData <- allData[,2:length(allData)]

#Set working directory to data file folder
setwd('D:/Documents/GW2CombatLogs/arcdps.cbtlogs/RaidHeroesLogs')
...
fileMover <- paste("file.rename(from = 'D:/Documents/GW2CombatLogs/arcdps.cbtlogs/RaidHeroesLogs/", fileNames, "', to = 'D:/Documents/GW2CombatLogs/arcdps.cbtlogs/RaidHeroesLogsArchive/", fileNames2, "')", sep = "")
eval(parse(text = fileMover))
 
#Write/append to data file
write.table(allData, "D:/Documents/GW2CombatLogs/arcdps.cbtlogs/allData.txt", sep = "\t", append = FALSE, col.names = FALSE)
